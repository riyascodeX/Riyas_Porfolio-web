    ...
    <div className="relative flex flex-col md:flex-row items-center justify-between mb-12 md:mb-16 w-full group">
      ...
      >
        <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 hover:border-red-500/30 hover:shadow-[0_15px_35px_rgba(255,42,42,0.1)] transition-all duration-500">
          ...
          {/* Conditional Upgraded Link Integration */}
          {item.liveLink && (
            <div className={`flex w-full mt-4 ${isEven ? 'md:justify-end' : 'md:justify-start'}`}>
              <a
                href={item.liveLink}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 bg-white/10 hover:bg-[#ff2a2a] text-white text-xs font-black uppercase tracking-wider py-2.5 px-4 rounded-xl border border-white/10 hover:border-[#ff2a2a] shadow-sm hover:shadow-[0_5px_15px_rgba(255,42,42,0.3)] transition-all duration-300"
              >
                <span>Link 😉</span>
                <svg 
